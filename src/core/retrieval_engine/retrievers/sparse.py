"""Sparse / FTS retriever — strategy=keyword."""
...
class SparseRetriever(BaseRetriever):
    _retriever_id = "sparse_fts"
    _supported_strategy = "keyword"

    def __init__(self, fts_store: Any = None) -> None:
...
def _to_raw(item: Any, retriever_id: str, query: RetrievalQuery) -> RawCandidate:
    ...
    raise TypeError(f"unsupported fts store result type: {type(item)!r}")
