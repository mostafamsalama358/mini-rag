"""Dense vector retriever — strategy=semantic."""
...
class DenseVectorRetriever(BaseRetriever):
    _retriever_id = "dense_vector"
    _supported_strategy = "semantic"
    ...
    async def retrieve(
        ...
        if self._store is not None:
            search = getattr(self._store, "search", None) or getattr(
                self._store, "similarity_search", None
            )
...
def _to_raw(item: Any, retriever_id: str, query: RetrievalQuery) -> RawCandidate:
    ...
    raise TypeError(f"unsupported vector store result type: {type(item)!r}")
