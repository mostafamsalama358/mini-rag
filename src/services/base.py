"""
services/base.py — Abstract Service Base Class
==============================================
.NET Equivalent: A base class shared by all service classes (NOT a web controller).

NOTE: This module moved here from `controllers/BaseController.py` as part of the
003 architecture refactor. Classes in this project's service layer are
business-logic services, not HTTP endpoint handlers. The HTTP layer lives in
`routes/*.py` (which are the true ASP.NET-style controllers).
